...
Полная диаграмма с примерами входов и выходов каждого этапа:
[docs/pipeline_data_flow.md](docs/pipeline_data_flow.md).
...
Во время запуска блок «Данные между этапами» показывает материалы источников,
структурированные факты, ограничения, основной и negative prompt, параметры генерации и
отчёты проверок. Поля выводятся с понятными подписями; исходный JSON каждого объекта можно
раскрыть отдельно. Ключи, токены и пароли в этом API автоматически скрываются.
...
Для постановочной демонстрации нажмите в панели `Ctrl+Alt+D`. Переключатель намеренно
скрыт, чтобы запись выглядела как обычный запуск. Выбранные настройки и промпт сохраняются,
но внешние API не вызываются и токены не требуются. Поля токенов и весь остальной UI при
этом не меняются. Примерно за 10 секунд панель проходит семь стадий pipeline, показывает
прогресс и журнал, после чего выводит корневой `fake.png`.
Кнопка «Скачать PNG» сохраняет результат. Повторное нажатие `Ctrl+Alt+D` возвращает реальный
режим. В скрытом режиме изображения можно прикрепить при любом выбранном генераторе: они
проверяются и сохраняются как контекст запуска. Другой итоговый файл можно указать через
